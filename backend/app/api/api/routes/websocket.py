...
from backend.app.repositories.redis.match_repo import get_match_state #ERRO de import
...
def find_your_mission(match: dict, player_id: str):
    ...
    for mission in missions:
        if mission.get("player_id") == player_id: #owner_id
            return mission.get("mission") or mission

    return None
#ERRO deveria estar no factory nao?
...
async def match_websocket(
    ...
    try:
        match = get_match_state(match_id) #ERROR pode pegar do repo?
        ...
        while True:
            ...
            if event_type == "choose_attack_option":
                ...
                response = resolve_attack_question(
                    ...
                    player_id=player_id,
                    answer=answer,
                )#ERR?O ta repetido no handlers.py
