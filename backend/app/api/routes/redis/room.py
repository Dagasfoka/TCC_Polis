...
from backend.app.schemas.redis.room import StartRoomRequest,JoinRoomRequest,PutReady,DeletePlayer
from backend.app.services.redis.room_service import create_room, join_room, start_game,put_ready,delete_player,get_room
...
async def start_game_route(
    ...
    return start_game(db, room_id, redis_client, data.host_id)
@router_room.post("/routes/{room_code}/ready")
async def ready(
    room_code: str,
    data: PutReady
):
    return put_ready(room_code,data.player_id)
@router_room.delete("/routes/{room_code}/delete")
async def delete(
    room_code : str,
    data: DeletePlayer,
):
    return delete_player(room_code,data.host_id,data.target_id)